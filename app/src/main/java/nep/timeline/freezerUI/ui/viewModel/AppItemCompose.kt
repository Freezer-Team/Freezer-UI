...
fun AppItemCompose(
    ...
    if ((!app.white || type == 1) && (app.black || type == 0)) {
        val otherConfig = if (SettingsConfigChecker.isFullyAutomatic()) app.bluetoothCheck else (((app.whiteProcCount + app.killProcCount > 0) || app.ignoreRecording || app.intervalUnfreeze || (app.backgroundIntent && !app.backgroundPlay) || app.notificationKeep || app.oomLevel != 0) && !SettingsConfigChecker.isLiteMode()) || app.bluetoothCheck || app.networkCheck || app.socket
    ...
    CustomBasicComponent(
        ...
        rightActions = {
            if ((app.white && type == 0) || (app.black && type == 1) || app.category != AppItem.Category.Undefined || app.allowAPIInjection) {
                /*Badge(
                ...
                StatusTag(
                    label = if (app.allowAPIInjection) "API" else (if (app.white && type == 0) stringResource(R.string.white_app) else if (app.black && type == 1) stringResource(R.string.black_app) else (if (app.category == AppItem.Category.Game) stringResource(R.string.game) else (if (app.category == AppItem.Category.Music) stringResource(R.string.music) else (if (app.category == AppItem.Category.Map) stringResource(R.string.map) else (if (app.category == AppItem.Category.Accessibility) stringResource(R.string.accessibility) else stringResource(R.string.other)))))),
                    backgroundColor = if (app.allowAPIInjection) Color.Cyan else (if (app.white && type == 0) (if (isSystemInDarkTheme()) Color.White else Color.Black) else if (app.black && type == 1) Color.Red else Color(60, 179, 113)),
                    contentColor = if (app.allowAPIInjection) Color.Black else (if (app.white && isSystemInDarkTheme()) Color.Black else Color.White)
                )
            }
        },
        onClick = {
