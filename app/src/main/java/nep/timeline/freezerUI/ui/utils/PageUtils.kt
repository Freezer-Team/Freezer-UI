...
fun AdaptiveTopAppBar(
    ...
    if (isWideScreen || BackgroundManager.forceSmallTop) {
        ...
        TopAppBar(
            title = title,
            subtitle = subtitle,
            color = color.copy(if (color.alpha == 0.0f) 0.0f else (if (BackgroundManager.currentUri == null) 1.0f else scrollBehavior.state.collapsedFraction)),
            scrollBehavior = scrollBehavior,
            navigationIcon = navigationIcon,
            actions = actions,
            bottomContent = bottomContent
...
fun BlurredBar(
    ...
    val blurRadius = (collapsedFraction.pow(2)) * BackgroundManager.topAppBarBlurRadius
                ...
                colors = BlurColors(
                    blendColors = listOf(
                        BlendColorEntry(color = MiuixTheme.colorScheme.surface.copy((scrollBehavior?.state?.collapsedFraction?.times(
                            BackgroundManager.topAppBarBlurAlpha
                        )) ?: BackgroundManager.topAppBarBlurAlpha)),
                    ),
                )
            )
