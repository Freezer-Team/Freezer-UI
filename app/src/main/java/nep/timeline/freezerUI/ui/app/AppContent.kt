package nep.timeline.freezerUI.ui.app
...
import nep.timeline.freezerUI.ui.custom.FloatingBottomBar
import nep.timeline.freezerUI.ui.custom.FloatingBottomBarItem
...
import nep.timeline.freezerUI.ui.page.FreezerLogPage
...
import nep.timeline.freezerUI.ui.utils.shouldExpandNavigationRail
...
private object UIConstants {
    ...
    val PAGE_TITLES = listOf(AppContext.context.getString(R.string.main), AppContext.context.getString(
        R.string.app_list), AppContext.context.getString(R.string.settings))
}
    ...
    val serializersModule = remember {
        SerializersModule {
            polymorphic(NavKey::class) {
                subclass(MainRoute.Main::class)
                subclass(MainRoute.About::class)
                subclass(MainRoute.Log::class)
                subclass(MainRoute.FreezerSettings::class)
                subclass(MainRoute.MMSettings::class)
                subclass(MainRoute.PUSHSettings::class)
                subclass(MainRoute.BatterySettings::class)
                subclass(MainRoute.ScriptSettings::class)
            }
        }
    }
    ...
    val navigationItems = remember {
        if (active)
            listOf(
                ...
                NavigationItem(UIConstants.PAGE_TITLES[1], MiuixIcons.File),
                NavigationItem(UIConstants.PAGE_TITLES[2], MiuixIcons.Settings)
            )
        else
            listOf(
                NavigationItem(UIConstants.PAGE_TITLES[0], MiuixIcons.HorizontalSplit),
    ...
    CompositionLocalProvider(
        ...
        val entryProvider = remember(backStack) {
            entryProvider<NavKey> {
                ...
                entry<MainRoute.FreezerSettings> {
                    FreezerSettingsPage(
                        active = active,
                        padding = padding,
                        scrollEndHaptic = appState.enableScrollEndHaptic
                ...
                entry<MainRoute.BatterySettings> {
                    BatterySettingsPage(
                        active = active,
                        padding = padding,
                        scrollEndHaptic = appState.enableScrollEndHaptic
                ...
                entry<MainRoute.ScriptSettings> {
                    ScriptSettingsPage(
                        active = active,
                        padding = padding,
                        scrollEndHaptic = appState.enableScrollEndHaptic
...
private fun WideScreenContent(
    ...
    val imageBackdrop = rememberBlurBackdrop()
...
private fun CompactScreenLayout(
    ...
    Scaffold(
        ...
        Box(modifier = if (appState.navigationStyle == 2) Modifier.layerBackdrop(kyantBackdrop) else (if (backdrop != null) Modifier.layerBackdrop(backdrop) else Modifier)) {
            PagerBackground(imageBackdrop)
            AppPager(
                snackbarHostState = snackbarHostState,
...
// Fixed wallpaper drawn once behind the pager (not per page, so it doesn't slide with page
// switches). It also feeds [imageBackdrop], the image-only blur source the pages' cards and
// top bars sample.
@Composable
private fun PagerBackground(imageBackdrop: LayerBackdrop?) {
    if (BackgroundManager.currentUri == null) return
    Image(
        painter = BackgroundManager.readImage(),
        contentDescription = null,
        modifier = Modifier
            .fillMaxSize()
            .then(if (imageBackdrop != null) Modifier.layerBackdrop(imageBackdrop) else Modifier),
...
private fun MainScreenBackHandler(
    ...
    val isPagerBackHandlerEnabled by remember {
        derivedStateOf {
            navigator.current() is MainRoute.Main && navigator.backStackSize() == 1 && mainState.selectedPage != 0
        }
    }
